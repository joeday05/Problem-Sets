...
#    word = word.lower()
    bonus = 0
    if len(word) == n: bonus = 50
    wordValue = 0
    for letter in word: wordValue += SCRABBLE_LETTER_VALUES[letter]
    wordValue *= len(word)
    wordValue += bonus    
    return wordValue
...
def updateHand(hand, word):
    ...
    return getFrequencyDict(sequence)
...
#hand = {'w':1, 's':1, 't':2, 'a':2, 'o':1, 'f':3}
#hand = {'n':1, 'e':1, 't':1, 'a':1, 'r':1, 'i':2}
#print compChooseWord(dealHand(HAND_SIZE), wordList, HAND_SIZE)
